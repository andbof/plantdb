...
# == PLANTDB ==
# === CHANGES ===
# Fix some str+int concatenation errors
# Raise a general exception when too much data is packed into the code
# === NOTES ===
# Seems to always encode QR in 8 bit mode
# Only QR type (1 <= typeNumber <= 40) and error correction level (L <= errorCorrectLevel <= H) can be changed
# For a list of typeNumbers, go to http://www.denso-wave.com/qrcode/vertable1-e.html
#
# To use:
# qr = QRCode(typeNumber, QRErrorCorrectLevel.{L,M,Q,H})
...
class QRUtil(object):
    ...
    def getMask(maskPattern, i, j):
        ...
        raise Exception("bad maskPattern:" + str(maskPattern));
    @staticmethod
...
class QRPolynomial:
    ...
    def __init__(self, num, shift):
        ...
        if (len(num) == 0):
            raise Exception(str(num.length) + "/" + str(shift))
...
class QRRSBlock:
    ...
    def getRSBlocks(typeNumber, errorCorrectLevel):
        ...
        if rsBlock == None:
            raise Exception("bad rs block @ typeNumber:" + str(typeNumber) + "/errorCorrectLevel:" + str(errorCorrectLevel))
